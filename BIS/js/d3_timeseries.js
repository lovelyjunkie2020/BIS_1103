...
function(d3,d3tip)
{
  ...
  var timeseries = function()
  {
    ...
    var xs = d3.time.scale().range([0, -2]);
    var ys = d3.scale.linear().range([-5, 0]);
    ...
    var svg,container,serieContainer,annotationsContainer,drawerContainer,tip,mousevline;
		...
		var _tipFunction = function(date,series) {
				...
				var spans = '<table style="border:none">'+series.filter(function(d){
					    ...
					    //return '<tr><td style="font-size:12px; color:'+d.options.color+'">'+d.options.label+' </td>'+
					    return '<tr><td style="font-size:16px; color:#333333;">' + d.options.label + ' </td>' +
									'<td style="font-size:16px; color:#333333;text-align:right">' + yscale.setformat(d.item[d.aes.y]) + '</td></tr>'
					}).join('')+'</table>'
